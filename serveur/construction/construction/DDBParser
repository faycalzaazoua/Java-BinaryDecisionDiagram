    ...
    private static Expression parse(String source, int offset) throws InvalidExpressionException {
        ...
        do {
            ...
            for (; pos < source.length(); pos++) {
                ...
                if (parentheses == 0) {
                    // En dehors de parenthÃ¨ses
                    if (c == '(') {
                        ouverture = pos;
                        parentheses++;
                    } else if (c == ')')
                        throw new InvalidExpressionException("ParenthÃ¨se fermante surnumÃ©raire Ã  la position " + (offset + pos));
                    else if (c == '!')
                        negation = !negation;
                    else if (c == '0' || c == '1') {
                        ...
                        throw new InvalidExpressionException("CaractÃ¨re inconnu Ã  la position " + (offset + pos));
                } else {
                    // Ã€ l'intÃ©rieur de parenthÃ¨ses
                    if (c == '(')
                        parentheses++;
                    else if (c == ')') {
                        parentheses--;
            ...
            if (parentheses > 0)
                throw new InvalidExpressionException("ParenthÃ¨se non fermÃ©e Ã  la position " + (offset + ouverture));
            else if (negation && expression == null)
                throw new InvalidExpressionException("NÃ©gation d'une expression vide Ã  la position " + (offset + pos));
            else if (expression == null)
                throw new InvalidExpressionException("Expression illÃ©galement vide Ã  la position " + (offset + pos));
            ...
            for (; pos < source.length(); pos++) {
                ...
                if (c == '.') {
                    ...
                    throw new InvalidExpressionException("OpÃ©ration inconnue Ã  la position " + (offset + pos));
            }
            if (operateur != null)
                operateurs.add(operateur);
        } while(operateur != null);

        // PrioritÃ©s des opÃ©rations :
